(function($) {
    ...
    $("a[data-type='quick']").on('click', async function(event) {
        let id = Number(this.id.substr(5));
        let type = this.id.substr(0,4);
        let xmlHttp = new XMLHttpRequest();
        xmlHttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                $('#QuicksPlaceholder').html(this.responseText);
                switch (type) {
                    case 'cart':
                ...
                $('#QuicksPlaceholder').html("");
            }
        };
        xmlHttp.open("GET","common/quicks.php?id="+id, true);
        xmlHttp.send();
    });
